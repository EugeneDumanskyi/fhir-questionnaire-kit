 ...
 * contract (docs/08-dom-contract.md §3), for the element's client tests and
 * its browser page: React's form (`packages/react/test/kinds.ts`), with the
 * repeating group required and at most two instances with one nested inside
 * it, so leaving it shows. Lenient, so the attachment degrades to
