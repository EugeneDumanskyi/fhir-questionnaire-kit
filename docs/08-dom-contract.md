...
**Status:** complete for every control kind, 2026-09-23 (M5). §3.1 and §3.2 are the S1 rows, which both renderers build. Both renderers build every row, §3.9's tier-3 chrome included, checked row by row against the model by `tests/browser/contract-rows.ts`: React from 2026-09-24 (M6), the element from 2026-09-25 (M7). The summary's `part` names follow §1's shared stem from 2026-09-25 (`06-roadmap.md` M7 plan D7); until then they were `error-summary*`. The contract suite covers each row as its renderer lands (§4). Anything not written here is not part of the contract. Class names, `part` names and the roles and ARIA attributes below are covered by semver from 1.0.0.
...
- **The leave rule (M5 plan D11).** `leave()` is called on `focusout` from the item root when `relatedTarget` is outside that root, including `null`. Focus moving between parts of one item (two radios, an entry and its unit) is not leaving it. Nor is an event the element's own patch sets off: Chromium fires `focusout` from a focused control the patch removes, such as a repeat instance's own remove button, where Firefox and WebKit fire nothing, so the element ignores focus events while it patches (M7 plan step 3b). A focus move the view asks for, to the error summary or an added or removed instance's neighbour, happens after the patch and is leaving, as in React. This is the one behaviour both renderers implement rather than read from the view: it needs the DOM.
...
| `errorSummary` (when not `null`) | `section`, first child of the form | `fhirq-summary` | `summary` | `id` = `errorSummary.id`, `tabindex="-1"`, `aria-labelledby` = `errorSummary.headingId` |
| `errorSummary.heading` | `h2` | `fhirq-summary-heading` | `summary-heading` | `id` = `errorSummary.headingId` |
| `errorSummary.entries` | `ul` | `fhirq-summary-list` | `summary-list` | — |
| each entry | `li` > `a` | `fhirq-summary-entry` > `fhirq-summary-link` | `summary-entry` > `summary-link` | `href` = `#` + `entry.focusId`; activating it moves focus to that id |
| an entry with `focusId: null` (a form-level issue) | `li`, text only, no link | `fhirq-summary-entry` | `summary-entry` | — |
...
In the element, the host's control is a custom element the host defined, made inside the shadow root. Its `props` property is set to `ControlProps` on each render of the item, and it may answer through `props` or by dispatching `fhirq-set` (`detail` = `set`'s argument), `fhirq-clear` and `fhirq-leave` on itself.
...
In development, after each render of the item, the renderer checks the first three duties in its own tree (the document, or the element's shadow root; a control a host renders in a portal still counts), and raises `control-contract` once per item and missing attribute: `path` the item's, `detail` the kind, `expected` one of `id`, `aria-invalid` or `aria-describedby` (`docs/07-api.md` §3). The element checks in a microtask after its paint, since a custom element may draw its `props` in a microtask of its own. A production build does not check.

## 4. What the contract suite asserts

On the demo (`fixtures/demo`), in each of three states (as loaded; answered, with every conditional item shown and a second medicine added; completion refused), both renderers produce the same tree of elements, classes, `part` names, roles and attributes; every id reference resolves in the same root and is described by the element it reaches; the input's `.value` and `.checked` match; and Playwright's ARIA snapshot (roles, accessible names, states) is identical. Ids themselves are compared by what they resolve to, never by value, since the prefixes differ.

Every row of §2 and §3, for all 18 control kinds, is also checked against the model a renderer drew: `tests/browser/contract-rows.ts` takes the view model and the `.fhirq-form` and returns each departure from this document. It checks tags, classes, `part` names, ids, ARIA state, text, values and selection, the heading level of repeat instances, and that no `style` is present. Given the kinds a host overrides, it checks §3.9's chrome for them instead. React runs it in Chromium on React 18 and 19, on first render and after each kind is used (`packages/react/test/browser/ui.test.tsx`), and the element the same way (`packages/element/test/browser/kinds.test.ts`); each runs it over its tier-3 overrides too (`controls.test.tsx`, `controls.test.ts`).

The `part` names are checked against this document itself: `tests/browser/isolation.spec.ts` parses every table's `part` column, each `part` attribute written in the prose and §1's class stems from this file, and finds all of them drawn by the element on the demo or a page with every kind, no undocumented name drawn, and every element's parts equal to its class stems (M7 AC-7). The cross-renderer suite on the demo is blocking from M7, in Chromium, in the required `Element gates` job, where the parts check runs in Chromium, Firefox and WebKit.
