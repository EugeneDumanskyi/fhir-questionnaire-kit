...
**Status:** complete for every control kind, 2026-09-23 (M5). §3.1 and §3.2 are the S1 rows, which both renderers build. React builds every row, §3.9's tier-3 chrome included, from 2026-09-24 (M6), checked row by row against the model by `tests/browser/contract-rows.ts`; the element builds every row but §3.9 from 2026-09-25 (M7 plan step 4), under the same check, and §3.9 with its tier 3 from 2026-09-25 (step 7). The summary's `part` names follow §1's shared stem from 2026-09-25 (`06-roadmap.md` M7 plan D7); until then they were `error-summary*`. The contract suite covers each row as its renderer lands (§4). Anything not written here is not part of the contract. Class names, `part` names and the roles and ARIA attributes below are covered by semver from 1.0.0.
