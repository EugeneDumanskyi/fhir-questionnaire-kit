...
**Step 3, split in two, 2026-09-25.** The keyed patcher was planned as one PR and built as two, to spend effort and tokens only where they were needed. The reconciler (3a, #72) needed max effort: every child list and focus safety rest on it. The kind descriptors over it (3b, #73) repeat one pattern once 3a had set it, so they ran at high. Each PR read its own bytes: 3a **23.74 kB**, 3b **24.65 kB**.

**Built on 2026-09-25, in PRs #70–#80.** Evidence per acceptance criterion:
- **AC-1.** `examples/element-embed/` is a plain `index.html` with one `<script src="fhirq-element.js">`, one `<fhir-questionnaire src="demo.json">` and a CSP `<meta>`, and no script of its own. `tests/browser/embed.spec.ts` serves it with no bundler, with the IIFE built by `scripts/build-element.mjs` itself, in Chromium, Firefox and WebKit. It renders from markup alone with exactly three requests (page, script, `demo.json`). Answering "Yes" shows the pain score, typed answers are taken, and a hidden answer is kept. There is no CSP violation, console error or warning.
- **AC-2.** `@fhirq/element` reads **27.28 kB** and the IIFE **27.48 kB**, production builds. That is over 24 kB with rung 2 built, so **ADR-0024 (rung 3)** sets the figures to **31.7 and 31.9 kB**: the reading, plus 3.21 kB for the theme M8 still grows, plus a 1.21 kB margin. Accepted 2026-09-25. Both are in `gated` from #80, checked by `pnpm measure --check` in `Engine gates`. The theme stays embedded.
- **AC-3.** `tests/browser/csp.spec.ts` runs the slice, the demo and the embed under `script-src 'self'; style-src 'self'`, with every conditional item answered, a second medicine added and completion refused. In Chromium, Firefox and WebKit it reads 0 violations, 0 `<style>`, 0 `style` attributes, 0 `el.style` writes and 2 adopted sheets, with the computed styles applied. The inline-style lint bans cover every file in `packages/element/src`.
- **AC-4.** `tests/browser/isolation.spec.ts`, in 3 engines. The first check compares a clean page with a hostile one, in two states. The hostile sheet has a `*` content-box rule with `!important`, bare element rules, and rules naming the kit's own classes. Every computed property of every part matches, and the screenshots are byte-identical. The one exception is the insets of the visually hidden status, which compute to where it sits on the page. The second check uses four host stylesheets: every computed property of the host's own elements is the same with and without the element, and the document gains no stylesheet. Inheritance does cross the boundary; see "Still open".
- **AC-5.** Covered by four tests:
  - `patch.test.ts`, 18 tests: with the focus rule removed, 4 of 6 reorder cases fail.
  - `identity.test.ts`: M5's identity sequence, carried to the DOM. A MutationObserver sees 0 mutations in every node the view kept.
  - `focus.test.ts`: items and repeat instances above and after the focused field are hidden, shown, added and removed. The field is never moved or removed, gets no `focusout`, and keeps its caret.
  - `caret.spec.ts`: every entry kind, both unit forms, a wall clock with `timeZone`, and a repeating question with a middle entry cleared key by key, in Chromium, Firefox and WebKit.

  The NFR-P-03 reading is in `03-nfr.md` §1: 1.0 ms median and 2.0 ms p95 of script at 500 items on the CI runner. **R4 is retired.**
- **AC-6.** `tests/browser/reconnect.spec.ts`, 3 engines, on the demo (the host's session) and the embed (a session from `src`). It removes and re-inserts the element 100 times, then does an HTMX-style swap:
  - listener counts are equal before and after: instrumented in every engine, and by CDP and `jsEventListeners` in Chromium;
  - after garbage collection, detached nodes are equal (Chromium);
  - answers, ids and values are unchanged, and one keystroke still makes exactly 1 cycle and 1 `fhirq-change`;
  - a leaky control subclass reads exactly +101 on each counter, so every counter can see a leak.
- **AC-7.** In `isolation.spec.ts`:
  - tokens set on the element reach the controls and labels, and `::part(label)` and `::part(add)` apply;
  - all 35 part names are parsed from `08-dom-contract.md` and drawn on the demo or the kinds page, with no undocumented name drawn and every element's parts equal to its class stems;
  - D7 aligned the summary's parts with the stem first.

  A token set on an *ancestor* does not reach the form; see "Still open".
- **AC-8.** The request-shape and failure tests:
  - `resolver.spec.ts`, 3 engines: markup alone makes one GET for `src` and one per distinct canonical, and nothing after answering, reconnecting or a new view;
  - the resolver contract suite covers success, nested `contains`, non-2xx, malformed JSON and abort on dispose.

  `no-io.test.ts` runs unbundled with every door locked. `fetch` is the only door touched, and each call's caller frame is `src/default-resolver.ts`. A control proves that a call from another file fails.
- **AC-9.** `tests/browser/two-elements.spec.ts`, 3 engines:
  - both roots draw the same ids and none repeats within a root or reaches `document`;
  - every id reference and summary link resolves in its own root;
  - typing into one element leaves the other's session unchanged, with 0 mutations in its shadow root and 0 `fhirq-change`.
- **AC-10.** **99.17 % line, 97.17 % branch**, from Vitest browser mode in Chromium (D8), held at 85 / 80 by `pnpm test:coverage:element` in `Element gates`.
- **Scope.**
  - Tier 3 (`controls.test.ts`): an accessible `calendar-date` override passes the label, error and `aria-invalid` tests. One without an `id` raises one `control-contract` naming `calendar-date` and `id`. Typing into one override writes nothing outside its item. The check is left out of the IIFE (`build-element.test.js`).
  - Locale: the `locale` → `lang` → `navigator.language` → `"en"` matrix (`locale.test.ts`).
  - Event payloads are plain data (`events.test.ts`).
  - Axe: 0 violations on the demo and the embed, loaded and refused, in 3 engines.
  - Symbols: 59 of 60, with the element at 2 (D4).
  - Form participation stays out of v1 (AT5).

**Element bytes, by step** (gzip, production builds from step 2):

| After | Element | IIFE |
|---|---:|---:|
| The M5 view under the spike renderer (development build) | 23.60 | 23.80 |
| 2, the production build | 23.59 | 23.57 |
| 3a, the keyed reconciler (#72) | 23.74 | 23.70 |
| 3b, the kind descriptors: entry kinds, groups, repeats (#73) | 24.65 | 24.63 |
| 4, the option and read-only kinds (#74) | 25.47 | 25.42 |
| 5, inputs, lifecycle and events (#75) | 26.72 | 26.92 |
| 6, the default resolver (#76) | 27.03 | 27.24 |
| 7, tier 3 (#77) | 27.28 | 27.48 |
| 8, 9: tests only (#78, #79) | 27.28 | 27.48 |

About 0.54 kB of step 5 is core's emission, which the events need. The renderer is about 4.4 kB against S1's 5.9 kB centre.
...
- **The detached-node count** is Chromium only (CDP). Firefox and WebKit get the listener and answer checks.
- **The cross-renderer contract spec** runs in Chromium only, since it compares markup.
- **A WebKit load stall.** Two two-element tests timed out together in one of four local runs, and never again in 64 repeats. It has not recurred in `Element gates`, which is watching for it.
- **Keystroke to paint** still has no device reading and no Firefox or WebKit reading.
- **Repeating-question entries stay keyed by position** (D10). It held: a cleared middle entry stays in the draft as `''`, and caret and nodes are kept in 3 engines.
- M5's carried items are unchanged: a draft does not block completion, and a decimal's scale after a restore.

**Found on the way.**
- **Chromium fires `focusout` from a focused node the patch removes; Firefox and WebKit fire nothing.** Probed in all three at 3a. It had two consequences:
  - A paint could start a cycle mid-patch, so a cycle raised during a paint is painted after it.
  - At 3b, removing a repeat instance with its own remove button read as leaving its group in Chromium alone, so a required group showed its error while focus was still inside it. The element now ignores focus events while it patches, and `08-dom-contract.md` §1's leave rule says so.
- **A node can change kind at the same path.** The view picks radios or a list by option count, so a value set that resolves to more than five options turns radios into a list. Items are keyed by kind and path, and a new key builds a new root. No view field was needed.
- **`lang="en_US"`, common on older pages, would make `Intl` throw** at the first formatted value. `locale.ts` skips a tag that is not well formed, with a regex checked against `Intl.getCanonicalLocales`, since that file may not use `Intl`.
- **The ADR-0012 note asked for "a typed error" without naming one.** Core gained `request-failed` and `FhirqError`'s `cause` (#75), which the value-set resolver reuses.
- **A tier-3 control can answer before its item is in the shadow root.** Its `fhirq-set`, `fhirq-clear` and `fhirq-leave` are therefore heard on the control itself, the one exception to root-delegated listeners. The development check runs a microtask after the paint, so a control that draws in a microtask of its own, as Lit does, is not flagged.
- **The platform reads lifecycle callbacks once, at `define`.** So the leak control for the reconnect test is a subclass under its own tag, not a patched prototype.
- **The status region kept the previous session's announcement** after `session` was replaced (3a). Fixed at step 5, where a new session or view resets the records.
- **Firefox keeps focus on the last control** when a test tabs past the end of the page, where Chromium and WebKit move it off the page. The caret test now leaves with Shift+Tab.
- **A local paint reading** had the element's 500-item median one frame later than React's (32 against 16 ms), with the extra time after the handlers. The CI runner reads 16 ms for both. It is not investigated.
...
| Architectural lint rules: no DOM in core, no network anywhere, no hard-coded strings, no deep imports | M0 (rules), tightened M2/M4/M5 as modules land · M6 no DOM in a React render, and ADR-0020's pair on the React adapter · M7 ADR-0020's pair on the element, with the browser's language read in one file | NFR-M-06 |
