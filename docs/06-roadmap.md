...
**Run on 2026-09-16; output `00-s1-architecture-and-bytes.md`, `08-dom-contract.md`.** Every acceptance criterion has its evidence: bytes measured per entry point and extrapolated two ways (AC-1); a budget verdict proposed in `03-nfr.md` §2, keeping the figures and R1 open, awaiting approval (AC-2); the field-list deny-list test passing, awaiting the reviewer's sign-off (AC-3); one contract suite identical across both renderers (AC-4); 0 axe violations in 24 runs (AC-5); SSR in Node and 0 hydration warnings on React 18 and 19 (AC-6); caret and focus kept on Chromium and WebKit (AC-7); strict CSP with adopted sheets on both engines (AC-8); and a go/no-go note recommending *continue* in `05-architecture.md` §11, awaiting sign-off (AC-9). No kill criterion triggered. Two findings need a decision before M5: ADR-0020's `display` field fails ADR-0007's review rule, and the view budget is the likeliest first overrun.
