...
| N3 | NFR-S-02/03/04 | Core ≤ 14 kB, view ≤ 5 kB, react ≤ 6 kB (excl. React, core and view), element ≤ 24 kB, `base.css` ≤ 4 kB, theme preset ≤ 3 kB, IIFE ≤ 30 kB. **S1 measured the slice on 2026-09-16** (element 5.84 kB standalone) **and extrapolated bands that straddle the view, react, element and IIFE budgets** (§2 note, `00-s1-architecture-and-bytes.md`). Proposed, awaiting approval: figures unchanged, still assumed; each is re-read when its budget gate switches on | Published competitive claim; re-baseline after the engine spike |
