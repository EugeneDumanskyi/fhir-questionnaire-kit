...
**Amendment note, accepted 2026-09-24 (`06-roadmap.md` M6 plan D2): what `controls` is keyed on.** The Decision writes `controls={{ date: MyDatePicker }}`, keyed by FHIR item type. Since M5 one item type maps to several control kinds (`choice` to five, by `itemControl` hint and option count), `ControlProps<K>` is typed by kind, and a view node carries its kind but not its item type. `controls` is therefore keyed by **control kind**: `controls={{ 'calendar-date': MyDatePicker }}`, in both renderers. Only the 13 answerable kinds can be overridden; `calculated`, `statement`, `group`, `repeating-group` and `unsupported` cannot. Where this ADR, AC-10.3.1 and M6 AC-7 say "item type", read "control kind". The development check names the kind and the missing attribute, so the Verification's tier-3 test overrides `calendar-date` and expects `calendar-date` and `id`. The check's finding is a core `Diagnostic` with code `control-contract`, delivered as ADR-0015's note describes. The obligations and the chrome the kit renders are unchanged.
