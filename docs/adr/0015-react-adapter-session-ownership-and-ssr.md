...
**Amendment note, accepted 2026-09-24 (`06-roadmap.md` M6 plan D1, D3): the resolver gate and the diagnostic channel.** Neither part changes the decision; both say how it is built on the core M4 left.
- **The resolver gate (D3).** Since M4, core calls the resolver inside `createSession` as the session enters `in-progress` (ADR-0005). Under React, a component-owned session is created during render, so an unwrapped resolver would run on the server, and twice under StrictMode. The adapter therefore passes core a wrapper around the host's resolver. The wrapper returns a promise that calls the host's resolver only once the component's mount effect opens the gate, and never if the signal is aborted first. The host's resolver never runs on the server or for a session StrictMode discards, so "resolution starts in an effect on the client, never during server render" holds as written, with no core change. Each set stays `pending` until then, on the server and in the first client render alike. A host-created session is the host's: its resolver runs when the host creates it, and the documentation says so.
- **The diagnostic channel (D1).** A component cannot append to `session.diagnostics`, so `controlled-value-replaced` and ADR-0013's `control-contract` are core `DiagnosticCode`s that the adapter raises itself. It delivers each as a core `Diagnostic` to an `onDiagnostic` option, and in development also calls `console.warn` with the code and detail only (NFR-X-04). `controlled-value-replaced` names no path; `control-contract` carries the node's path, the control kind as `detail` and the missing attribute as `expected`.
